...
module.exports.move = function move(source, destination, callback) {
  fs.lstat(source, function (err, stats) {
    ...
    if (!stats.isDirectory()) {
      async.series([
        async.apply(fstools.remove, destination),
    ...
    var basepath_re = new RegExp('^' + source + '/?');

    fstools.walk(source, function (src, lstat, next) {
      var dst = path_join(destination, src.replace(basepath_re, ''));

      async.series([
        async.apply(fstools.remove, dst),
