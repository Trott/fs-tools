...
fstools.walk = function (path, pattern, iterator, callback) {
  ...
  path_exists(path, function (exists) {
    ...
    // start walking
    walk_recursive(path_normalize(path), function (path, stats, next) {
      // call iterator
      if (match(path)) {
...
fstools.remove = function (path, callback) {
  ...
  fs.lstat(path, function (err, stats) {
    ...
    async.series([
      async.apply(walk_flat, path, function (sub_path, stats, next) {
        fstools.remove(sub_path, next);
      }),
...
fstools.copy = function (src, dst, callback) {
  ...
  fs.lstat(src, function (err, stats) {
    ...
    fstools.mkdir(dirname(dst), function (err) {
      ...
      if (stats.isDirectory()) {
        async.series([
          async.apply(fs.mkdir, dst, '0755'),
          function (next) {
            walk_flat(src, function (sub_src, sub_stats, sub_next) {
              fstools.copy(sub_src, dst + sub_src.replace(src, ''), sub_next);
            }, next);
          },
          chmod
        ], done);
