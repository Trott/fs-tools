...
var cp = exports.cp = function cp(src, dst, callback) {
  ...
  var ifd = fs.createReadStream(src, options);
  ifd.on('error', callback);
  mkdir_p(dirname(dst), function(err) {
    if (err) { callback(err); return; }
    var ofd = fs.createWriteStream(dst, options);
    ofd.on('error', callback);
    ofd.on('end', callback);
    ifd.pipe(ofd);
  });
};
 ...
 *  Mimicks unix "cp -a".
 *
 *  TODO: implement via `walk`.
 **/
var cp_a = exports.cp_a = function cp_a(src, dst, callback) {
  require('child_process').exec('cp -a "' + src + '" "' + dst + '"', callback);
};
