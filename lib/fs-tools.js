...
// copies src to dst with given mode (for dst)
var copy = function copy(src, dst, mode, callback) {
  var ifd, ofd;

  // create streams
  ifd = fs.createReadStream(src, {bufferSize: 64 * 1024}).on('error', callback);
  ofd = fs.createWriteStream(dst, {mode: mode}).on('error', callback).on('end', callback);
...
var _cp_a = function _cp_a(src, dst, stats, callback) {
  ...
  if (stats.isFile()) {
    copy(src, dst, mode, function (err) {
      if (err) {
        callback(err);
        return;
      }
...
var cp = exports.cp = function cp(src, dst, callback) {
  mkdir_p(dirname(dst), function(err) {
    ...
    copy(src, dst, 436 /* 0664 */, callback);
  });
};
